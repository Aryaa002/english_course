...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($judul) || empty($deskripsi) || empty($kategori)) {
        ...
        $video_pembelajaran_path = $materi['video_pembelajaran'] ?? '';
        ...
        $stmt = $pdo->prepare("UPDATE materi SET 
                              ...
                              tingkat = ?, video_url = ?, audio_url = ?, video_pembelajaran = ? 
                              WHERE id = ?");
        
        if ($stmt->execute([$judul, $deskripsi, $konten, $kategori, $tingkat, 
                           $video_final, $audio_final, $video_pembelajaran_final, $id])) {
...
<head>
    ...
    <title>Edit Materi - English Course</title>
            ...
            <div class="top-bar">
                <div class="page-title">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h4><i class="fas fa-edit" style="color: #F4B41A;"></i> Edit Materi</h4>
                            <p>Edit materi yang sudah ada</p>
                        </div>
                    </div>
                </div>
                <div class="user-info">
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <?php if(!empty($materi['video_pembelajaran']) && file_exists('../' . $materi['video_pembelajaran'])): ?>
                                <div class="alert alert-info mt-2">
                                    <i class="fas fa-check-circle"></i> 
                                    Video saat ini: <?php echo basename($materi['video_pembelajaran']); ?>
                                </div>
                            <?php endif; ?>
                            <input type="text" class="form-control" id="video_pembelajaran" name="video_pembelajaran" 
                                   value="<?php echo htmlspecialchars($materi['video_pembelajaran'] ?? ''); ?>"
                        ...
                        <div class="col-md-12 mb-3">
                            <label>Upload Video Pembelajaran Baru</label>
